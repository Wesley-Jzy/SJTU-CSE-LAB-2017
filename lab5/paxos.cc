...
proposer::run(int instance, std::vector<std::string> cur_nodes, std::string newv)
{
  ...
  if (prepare(instance, accepts, cur_nodes, v)) {
    for (int j = 0; j < accepts.size(); j++) {
      std::cout << "ACCEPT: " << accepts[j] << std::endl;
    }
         ...
         std::string &v)
{
  ...
  for (unsigned int i = 0; i < nodes.size(); i++) {
    ...
    if(h.safebind()) {
      ...
      int ret = h.safebind()->call(paxos_protocol::preparereq, me, a, r, rpcc::to(1000));
      VERIFY(pthread_mutex_lock(&pxs_mutex) == 0);
      if (ret == paxos_protocol::OK) {
        if(!r.oldinstance) { 
          if(r.accept) {
            accepts.push_back(nodes[i]); 
            /* Put this Node in the list of acceptors */
            if(r.n_a > max) {
              max = r.n_a;   
              v = r.v_a;
            }
        ...
        std::vector<std::string> nodes, std::string v)
{
  ...
  for (unsigned int i = 0; i < nodes.size(); i++) {
    /* call the node's rpcs */
    handle h(nodes[i]);
    if(h.safebind()){
      /* construct res */
      bool r;
      VERIFY(pthread_mutex_unlock(&pxs_mutex) == 0);
      ret = h.safebind()->call(paxos_protocol::acceptreq, me, a, r, rpcc::to(1000));
      VERIFY(pthread_mutex_lock(&pxs_mutex) == 0);
      /* put accept node into set */
      if (r) {
        accepts.push_back(nodes[i]);
      }
    }
	      ...
	      std::string v)
{
  ...
  for (unsigned int i = 0; i < accepts.size(); i++) {
    ...
    if(h.safebind()){
      ...
      int r;
      VERIFY(pthread_mutex_unlock(&pxs_mutex) == 0);
      ret = h.safebind()->call(paxos_protocol::decidereq, me, a, r, rpcc::to(1000));
      VERIFY(pthread_mutex_lock(&pxs_mutex) == 0);
    } 
  }
}
