...
disk::disk(disk* old)
{
  ...
  for (id = 0; id < BLOCK_NUM; id++) {
    old->copy(id, blocks[id]);
  }
}

void 
disk::copy(uint32_t id, unsigned char *buf) {
  memcpy(buf, blocks[id], BLOCK_SIZE);
}

disk::disk(bool verify) {
  if (verify) {
...
{
  ver++;
  if (ver < VERSION_NUM && version[ver]) {
    d = version[ver];
  }
}
...
block_manager::read_block(uint32_t id, char *buf)
{
  ...
  d->read_block(id, disk_buf);
  verify->read_block(id, verify_buf);
  ...
  if (!isCorrect) {
    printf("something wrong happened in this block\n");
    d->write_block(id, verify_buf);
    memcpy(buf, verify_buf, BLOCK_SIZE);
  } else {
    memcpy(buf, disk_buf, BLOCK_SIZE);
  }
}
